...
Vue.component('gui-main', {
    ...
    methods: {
        ...
        editSchema: function (key) {
            ...
            let schemaEditorClose = function () {
                this.schemaEditor.destroy();
                $('#schemaModal').removeClass('is-active');
            }.bind(this);
            let schemaEditorSave = function () {
        ...
        renderOutput: function ($event) {
            ...
            let output = JSON.stringify(def, null, 4);
            $('#pretty-json').html(output);
            let clippy = new Clipboard('#copy-json');
            setTimeout(function () {
        ...
        renderOutputYaml: function ($event) {
            ...
            var def = this.$root.postProcessDefinition();
            try {
                let output = jsyaml.dump(def);
            }
            catch (ex) {
                alert(ex.message);
            }
            $('#pretty-yaml').html(output);
            let clippy = new Clipboard('#copy-yaml');
            setTimeout(function () {
                $('pre code').each(function (i, block) {
                    hljs.highlightBlock(block);
                });
